...
beforeAll(async () => {
  ...
  let loyaltyProgramInput = {
    ...
    //   type: "CUSTOM",
    //   ruleConfiguration: {},
    //   ruleExpression: "(Loyalty['totalAmount'] >= 100)?10:0"
    // },
...
describe("Issue loyalty transaction", () => {
  let customerLoyaltyInput = {
    externalCustomerId: externalCustomerId,
    // organizationId: user.organization.id,
    loyaltyCardCode: loyaltyCardCode //"RED001"
  };
  //issuePointsInput total amount = 100
  ...
  test("Issue, Issue with valid rule(Custom rule-> meamns issue points based on calculation)", async () => {
    const manager = getManager();
    ...
    console.log("Issue Points ..", issuePoints);
  ...
  test("Issue, Burn", async () => {
    const manager = getManager();
    let issuePointsInput2 = {
      externalCustomerId: externalCustomerId,
      loyaltyCode: loyaltyCode,
      // loyaltyProgram.code,
      loyaltyCardCode: loyaltyCardCode,
    ...
    let issuePoints3: any = await loyaltyTransactionService.issuePoints(
      manager,
      injector,
      issuePointsInput2
    );
    expect(issuePoints3.earnedPoints).toBe(12);
    // let getTranasctionObject = await loyaltyTransactionService.getLoaltyTransaction(manager, injector, issuePointsInput2.externalCustomerId, issuePoints3.loyaltyCardCode);
    // console.log("get trans obj..", getTranasctionObject)

    // let getTransactionLedgerRecords = await loyaltyTransactionService.getLedgerRecordsWithReferenceId(manager, getTranasctionObject[0].id)
    // console.log("...", getTransactionLedgerRecords)
    let burnPointsInput = {
      externalCustomerId: externalCustomerId,
      loyaltyCode: loyaltyCode,
      // issuePointsInput2.loyaltyCode,
      loyaltyCardCode: loyaltyCardCode,
      // issuePointsInput2.loyaltyCardCode,
      statusCode: "101",
      externalTransationId: "",
      loyaltyReferenceId: issuePointsInput2.loyaltyReferenceId,
      burnFromLoyaltyCard: true,
      organizationId: user.organization.id,
      transactionData: {
        transactionType: "DELIVERY_ORDER",
    ...
    // let getTranasctionObject = await loyaltyTransactionService.getLoaltyTransaction(manager, injector, burnPointsInput.externalCustomerId, burnPoints.loyaltyCardCode);
    // console.log("get trans obj..", getTranasctionObject)

    // let getTransactionLedgerRecords: any = await loyaltyTransactionService.getLedgerRecordsWithReferenceId(manager, getTranasctionObject[0].id)
    // console.log("getTransactionLedgerRecords", getTransactionLedgerRecords);
    // expect(getTransactionLedgerRecords[0].details).toEqual({ "1": 8 })

    // let customerLoyaltyInput = {
    //   externalCustomerId: "9985421502",
    //   organizationId: user.organization.id,
    //   loyaltyCardCode: "RED001"
    // };
    // let getCustomer: any = await customerLoyaltyService.getCustomerLoyalty(manager, injector, customerLoyaltyInput)
    // console.log("Get customer 3..", getCustomer);
    // expect(getCustomer.overallPoints).toBe(2)
  });
  test("Issue with invalid rule(Expecting error-> rule does not satisfy)", async () => {
    const manager = getManager();
    let externalCustomerId2 = "+918888888888";
    let createCustomerInput = {
      firstName: chance.string({ length: 10 }),
      lastName: chance.string({ length: 10 }),
      email: "pavani@getwalk.in",
      phoneNumber: externalCustomerId2,
      gender: "MALE",
      dateOfBirth: "",
      externalCustomerId: externalCustomerId2,
      customerIdentifier: externalCustomerId2,
      organization: user.organization.id,
      status: "ACTIVE"
    };
    let customer = await customerService.createOrUpdateCustomer(
    ...
    let issuePoints: any = loyaltyTransactionService.issuePoints(
      manager,
      injector,
      { ...issuePointsInputGlobal2, organizationId: user.organization.id }
    );
    await expect(issuePoints).rejects.toThrowError(
      "Loyalty rules are not satisfied."
    );
  });
});
