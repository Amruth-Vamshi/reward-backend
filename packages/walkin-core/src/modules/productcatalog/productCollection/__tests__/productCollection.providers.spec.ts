...
const createCollection = async manager => {
  ...
  const createdcollection = await collectionProvider.createCollection(
    manager,
    {
      name
    },
...
describe("Should create a product", () => {
  test("should Create a product for valid input", async () => {
    ...
    const createProductCollection = await productCollectionProvider.addProductsToCollection(
      manager,
      {
        productId: [createdProduct.id, createdProduct1.id],
        collectionCode: createdCollection2.code
      },
      user.organization.id
    );
...
describe("Should remove a product from collection", () => {
  test("Should remove a product from collection ", async () => {
    ...
    const removeCollectionFromProduct = await productCollectionProvider.removeProductsFromCollection(
      manager,
      {
