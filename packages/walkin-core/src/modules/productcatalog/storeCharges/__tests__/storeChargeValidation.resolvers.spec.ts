...
import * as storeChargeResolvers from "../storeCharge.resolvers";
...
describe("Create store charge", () => {
    test("Should FAIL to create DELIVERY_CHARGE charge type if it already exists for a store", async () => {
        ...
        const deliveryCharge = await storeChargeResolvers.default.Mutation.createStoreCharges(
            { user },
            {
                input: {
            ...
            { injector: StoreChargeModule.injector }
        );
        expect(deliveryCharge).toBeDefined();

        try {
            await storeChargeResolvers.default.Mutation.createStoreCharges(
                { user },
                {
                    input: {
                        chargeType: STORE_CHARGE_TYPE.DELIVERY_CHARGE,
    ...
    test("Should FAIL to update DELIVERY_CHARGE charge type if it already exists for a store", async () => {
        const manager = getManager();
        const taxType = await createCustomTaxType(manager);
        const storeFormatCode = chance.string({ length: 5 });
        const name = chance.string();
        ...
        const newDeliveryCharge = await storeChargeResolvers.default.Mutation.createStoreCharges(
            { user },
            {
                input: {
        ...
        expect(newDeliveryCharge).toBeDefined();

        try {
            await storeChargeResolvers.default.Mutation.updateStoreCharges(
                { user, application },
                {
                    id: newDeliveryCharge.id,
                    input: {
                        chargeType: STORE_CHARGE_TYPE.DELIVERY_CHARGE,
                        chargeValueType: STORE_CHARGE_VALUE_TYPE.ABSOLUTE,
                        storeId: createStore.id,
    ...
    test("Should create PACKAGING_CHARGE charge type after creating DELIVERY_CHARGE charge type for a store", async () => {
        ...
        const storeChargeValue = await storeChargeResolvers.default.Mutation.createStoreCharges(
            { user },
            {
                input: {
                    chargeType: STORE_CHARGE_TYPE.PACKAGING_CHARGE,
