...
import { resolvers as partnerResover } from "../partner.resolvers";
...
describe("Should Create a Partner", () => {
  test("should Create a partner with valid input", async () => {
    ...
    const createPartner = await partnerResover.Mutation.addPartner(
      { user },
      { input: partner },
      {
        injector: PartnerModule.injector
      }
    );
  ...
  test("should Fail to create with duplicate partner type", async () => {
    ...
    try {
      const createPartner1 = await partnerResover.Mutation.addPartner(
        { user },
        { input: partner },
        {
          injector: PartnerModule.injector
...
describe("Should update a partner type", () => {
  test("should update with vaid input", async () => {
    ...
    const updateSchema = await partnerResover.Mutation.updatePartner(
      { user },
      { input: updatedSchema },
      { injector: PartnerModule.injector }
    );
  ...
  test("should fail to update with duplicate partner type", async () => {
    ...
    const createPartner2 = await partnerResover.Mutation.addPartner(
      { user },
      { input: partner2 },
      {
        injector: PartnerModule.injector
...
describe("Should remove a partner", () => {
  test("should remove a partner with valid input", async () => {
    ...
    const removedPartner = await partnerResover.Mutation.removePartner(
      { user },
      {
        input: {
          id: createPartner.id
  ...
  test("should fail to remove partner with invalid input", async () => {
    const manager = getManager();
    try {
      await partnerResover.Mutation.removePartner(
        { user },
        {
          input: {
            id: chance.guid()
...
describe("Should fetch a partner", () => {
  test("should fetch a partner for valid input", async () => {
    ...
    const fetchPartner = await partnerResover.Query.getPartner(
      { user },
      {
        filter: {
          id: createPartner.id
