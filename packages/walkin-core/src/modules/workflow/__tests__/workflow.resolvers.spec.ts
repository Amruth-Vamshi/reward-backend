...
import { WORKFLOW_ENTITY_TYPE, STATUS } from "../../common/constants";
...
describe("WorkFlows Resolver test souce TestSuite", () => {
  test("Create Workflow Resolver Test", async () => {
    const newWorkFlow = {
      name: "TEST_WORK_FLOW",
    ...
    let createdWorkFlow = await resolvers.Mutation.createWorkflow(
      {},
      { input: newWorkFlow },
      { injector: workflowModule.injector }
    );
    ...
    getCreatedWorkflow = await resolvers.Query.workflow(
      {},
    ...
    expect(getCreatedWorkflow.organizationId).toEqual(user.organization.id);
  });

  test("Update Workflow resolver Test", async () => {
    const entityManager = getManager();

    const workflow = await resolvers.Query.workflowByName(
      {},
      {
        name: "TEST_WORK_FLOW",
        organizationId: user.organization.id
    ...
    let udpatedWorkflow = await resolvers.Mutation.updateWorkflow(
      {},
      { input: workflow },
      { injector: workflowModule.injector }
    );
    ...
    expect(udpatedWorkflow.organizationId).toEqual(user.organization.id);
    ...
    let newWorkflow = await resolvers.Mutation.updateWorkflow(
      {},
      { input: udpatedWorkflow },
      { injector: workflowModule.injector }
    );
    ...
    expect(newWorkflow.organizationId).toEqual(user.organization.id);
    expect(newWorkflow).toHaveProperty("name");
    expect(newWorkflow.name).toEqual(udpatedWorkflow.name);
  });

  test("Create WorkflowProcess resolver Test", async () => {
    const entityManager = getManager();
    ...
    const createdWorkflowProcess = await resolvers.Mutation.createWorkflowProcess(
      {},
      { input: newWorkflowProcess },
      { injector: workflowModule.injector }
    );
    ...
    let getCreatedWorkflowProcess = await resolvers.Query.workflowProcess(
      {},
      { id: createdWorkflowProcess.id },
      { injector: workflowModule.injector }
    );
    ...
    let getWorkflowProcessByName = await resolvers.Query.workflowProcessByName(
      {},
      { name: newWorkflowProcess.name, workflowId: workflow.id },
      { injector: workflowModule.injector }
    );
  ...
  test("Update WorkflowProcess resolver  Test", async () => {
    const workflow = await resolvers.Query.workflowByName(
      {},
      {
        name: "TEST_WORK_FLOW",
    ...
    let updatedWorkflowProcess = await resolvers.Mutation.updateWorkflowProcess(
      {},
      { input: getWorkflowProcessByName },
      { injector: workflowModule.injector }
    );
  ...
  test("Create WorkflowState resolver test", async () => {
    const workflow = await resolvers.Query.workflowByName(
      {},
      {
        name: "TEST_WORK_FLOW",
    ...
    let createdWorkflowState = await resolvers.Mutation.createWorkflowState(
      {},
    ...
    let getCreatedWorkflowState = await resolvers.Query.workflowState(
      {},
      { id: createdWorkflowState.id },
      { injector: workflowModule.injector }
    );
    ...
    let udpatedWorkflowState = await resolvers.Mutation.updateWorkflowState(
      {},
      { input: getCreatedWorkflowState },
      { injector: workflowModule.injector }
    );
    ...
    let createdState2 = await resolvers.Mutation.createWorkflowState(
      {},
      { input: newWorkflowState2 },
      { injector: workflowModule.injector }
    );
  ...
  test("test getWorkflowStatesByWorkflowId", async () => {
    ...
    let workflowStatesByWorkflowId = await resolvers.Query.workflowStates(
      {},
      { workflowId: workflow.id },
      { injector: workflowModule.injector }
    );
    ...
    expect(workflowStatesByWorkflowId.length).toBeGreaterThanOrEqual(1);
  });

  test("Create WorkflowProcessTransitions resolver test", async () => {
    const workflow = await resolvers.Query.workflowByName(
      {},
      {
        name: "TEST_WORK_FLOW",
    ...
    let createdStartState = await resolvers.Mutation.createWorkflowState(
      {},
      { input: newWorkflowStartState },
      { injector: workflowModule.injector }
    );
    ...
    let createdEndState = await resolvers.Mutation.createWorkflowState(
      {},
      { input: newWorkflowEndState },
      { injector: workflowModule.injector }
    );
    ...
    let createdWorkflowProcessTransition = await resolvers.Mutation.createWorkflowProcessTransition(
      {},
      { input: newWorkflowProcessTransition },
      { injector: workflowModule.injector }
    );
    ...
    let getWorkflowProcessTransition = await resolvers.Query.workflowProcessTransition(
      {},
      { id: createdWorkflowProcessTransition.id },
      { injector: workflowModule.injector }
    );
    ...
    let createdAbortState = await resolvers.Mutation.createWorkflowState(
      {},
      { input: newWorkflowAbortState },
      { injector: workflowModule.injector }
    );
    ...
    let updatedWorkflowProcessTransition = await resolvers.Mutation.updateWorkflowProcessTransition(
      {},
      { input: getWorkflowProcessTransition },
      { injector: workflowModule.injector }
    );
    ...
    let workflowProcessTransitionsByWorkflowProcessId: any = await resolvers.Query.workflowProcessTransitions(
      {},
      { workflowProcessId: getWorkflowProcessByName.id },
      { injector: workflowModule.injector }
    );
  ...
  test("CreateWorkflowWithChildren resolvers test", async () => {
    const entityManager = getManager();
    ...
    let createdWorkflowWithChildren = await resolvers.Mutation.createWorkflowWithChildren(
      {},
      { input: newWorkFlowWithChildren },
      { injector: workflowModule.injector }
    );
    ...
    let foundWorkflowProcesses = await resolvers.Query.workflowProcesses(
      {},
      { workflowId: createdWorkflowWithChildren.id },
      { injector: workflowModule.injector }
    );
    ...
    for (let record in foundWorkflowProcesses) {
      ...
      let foundTransistions = await resolvers.Query.workflowProcessTransitions(
        {},
        { workflowProcessId: foundWorkflowProcess.id },
        { injector: workflowModule.injector }
      );
    ...
    let foundWorkflowStates = await resolvers.Query.workflowStates(
      {},
      { workflowId: createdWorkflowWithChildren.id },
      { injector: workflowModule.injector }
    );
  ...
  test("CreateWorkflowEntity resolvers test", async () => {
    const workflow = await resolvers.Query.workflowByName(
      {},
      {
        name: "TEST_WORK_FLOW",
    ...
    let workflowEntity = await resolvers.Mutation.createWorkflowEntity(
      {},
      { input: newWorkflowEntity },
      { injector: workflowModule.injector }
    );
  ...
  test("UpdateWorkflowEntity test", async () => {
    ...
    let updatedWorkflowEntity = await resolvers.Mutation.updateWorkflowEntity(
      {},
      { input: workflowEntity },
      { injector: workflowModule.injector }
    );
    ...
    let foundWorkflowEntity = await resolvers.Query.workflowEntity(
      {},
      { id: updatedWorkflowEntity.id },
      { injector: workflowModule.injector }
    );
  ...
  test("addWorkflowEnityTransitionStatus  resolvers test", async () => {
    const workflow = await resolvers.Query.workflowByName(
      {},
      {
        name: "TEST_WORK_FLOW",
    ...
    const createdWorkflowEntityTransition = await resolvers.Mutation.addWorkflowEnityTransitionStatus(
      {},
      { input: newWorkflowentityTransistion },
      { injector: workflowModule.injector }
    );
    ...
    let workflowEntityTransition = await resolvers.Query.workflowEntityTransition(
      {},
      { id: createdWorkflowEntityTransition.id },
      { injector: workflowModule.injector }
    );
    ...
    let foundWorkflowEntityTransition = await resolvers.Query.workflowEntityTransitionByEntityId(
      {},
      { workflowEntityId: workflowEntity.id },
      { injector: workflowModule.injector }
    );
    ...
    let foundWorkflowEntityTransistionHistory = await resolvers.Query.workflowEntityTransitionHistory(
      {},
      { workflowEntityId: workflowEntity.id },
      { injector: workflowModule.injector }
    );
  ...
  test("CreateWorkflowRoute test", async () => {
    ...
    let rule: CoreEntities.Rule = CoreEntities.Rule.create({
      name: "TestRule",
      ruleConfiguration: {},
      ruleExpression: {}
    });

    let createdRule = await CoreEntities.Rule.save(rule);

    let createdWorkflowRoute = await resolvers.Mutation.createWorkflowRoute(
      {},
      {
        input: {
          entityType: WORKFLOW_ENTITY_TYPE.CAMPAIGN,
    ...
    let foundWorkflowRoute = await resolvers.Query.workflowRoute(
      {},
      { id: createdWorkflowRoute.id },
      { injector: workflowModule.injector }
    );

    expect(foundWorkflowRoute).toBeDefined();
    expect(foundWorkflowRoute).toHaveProperty("id");
    expect(foundWorkflowRoute.workflow.id).toBe(workflow.id);
    expect(foundWorkflowRoute.id).toEqual(createdWorkflowRoute.id);

    let foundWorkflowRoutes = await resolvers.Query.workflowRoutes(
      {},
      {
        organizationId: user.organization.id,
    ...
    // SKipping this test case since we create different workflows for the ame
    // for (const workflowRoute of foundWorkflowRoutes) {
    //   expect(workflowRoute.workflow.id).toBe(workflow.id);
    // }
  ...
  test("UpdateWorkflowRoute Resolver test", async () => {
    const workflow = await resolvers.Query.workflowByName(
      {},
      {
        name: "TEST_WORK_FLOW",
    ...
    let createdWorkflowRoute = await resolvers.Mutation.createWorkflowRoute(
      ...
      {
        input: {
          entityType: WORKFLOW_ENTITY_TYPE.OFFER,
          ruleId: createdRule.id,
          workflowId: workflow.id,
          organizationId: user.organization.id
        }
    ...
    expect(createdWorkflowRoute.status).toEqual(STATUS.ACTIVE);
    ...
    let updatedWorkflowRoute = await resolvers.Mutation.updateWorkflowRoute(
      {},
      {
        input: {
          id: createdWorkflowRoute.id,
